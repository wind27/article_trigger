...
public class DocumentArticleTransfer {
	...
	public static Article document2Article(Document doc) {
		...
		if(doc.getString("tags")!=null) {
			String tmp = doc.getString("tags");
			article.setTags(tmp!=null?tmp:"");
		}
		if(doc.getString("from")!=null) {
			String tmp = doc.getString("from");
			article.setFrom(tmp!=null?tmp:"");
		}
		if(doc.getString("original_link")!=null) {
			String tmp = doc.getString("original_link");
			article.setOriginalLink(tmp!=null?tmp:"");
		}
	...
	public static Document article2Document(Article article) {
		...
		doc.put("from", article.getFrom());
		doc.put("id", article.getId());
		doc.put("uid", article.getUid());
		doc.put("tags", article.getTags());
		...
		doc.put("original_link", article.getOriginalLink());
